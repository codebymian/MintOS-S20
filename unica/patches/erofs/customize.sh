...
# Safe integer check for API level
if [ "${TARGET_PRODUCT_SHIPPING_API_LEVEL:-0}" -ge 33 ]; then
...
if [[ "$TARGET_OS_FILE_SYSTEM" != "erofs" ]]; then
    _LOG "TARGET_OS_FILE_SYSTEM is not set to erofs"
    unset -f _LOG
    return 0
fi
...
while IFS= read -r f; do
    ...
    RAMDISK_FORMAT=""
    if [[ "$(READ_BYTES_AT "$f" 0 2)" == "1f8b" ]]; then
        RAMDISK_FORMAT="gz"
    elif [[ "$(READ_BYTES_AT "$f" 0 4)" == "04224d18" ]]; then
        RAMDISK_FORMAT="lz4"
    fi
    ...
    LOG "  -> Detected ramdisk format: $RAMDISK_FORMAT"

    EVAL "mkdir -p \"$TMP_DIR/out/ramdisk_extracted\""
    if [[ "$RAMDISK_FORMAT" == "gz" ]]; then
        EVAL "gzip -d < \"$f\" | cpio --quiet -i -D \"$TMP_DIR/out/ramdisk_extracted\""
    else
        EVAL "lz4 -d < \"$f\" | cpio --quiet -i -D \"$TMP_DIR/out/ramdisk_extracted\""
    fi
...
unset -f _LOG PATCH_FSTAB READ_BYTES_AT
